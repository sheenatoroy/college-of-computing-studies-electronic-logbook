...
if (isset($_POST['insertAppointment'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $query = "INSERT INTO appointments (`student_id`, `prof_id`, `type_of_concern`, `specific_concern`, `detailed_concern`, `appointment_status`, `remarks`, `time_start`, `time_end`, `day`, `appoint_by`) 
                    VALUES ('$student_id', '$prof_id', '$type_of_concern', '$specific_concern', '$detailed_concern', '$appointment_status', '$remarks', '$formattedDateTimeStart', '$formattedDateTimeEnd', '$day', '$appoint_by')";
            ...
            ";
                ...
                if ($mail->send()) {
                    ...
                    if (mysqli_stmt_execute($updateStmt)) {
                        // Update successful
                        echo '<script> alert("Appointment Successfully Saved and Email Sent"); window.location.href = "../faculty-pending-appointment.php"; </script>';
                    } else {
                        // Handle the error
                        echo '<script> alert("Appointment Saved, Email Sent, but Evaluation Status Not Updated. Error: ' . mysqli_error($conn) . '"); window.location.href = "../faculty-set-appointment.php"; </script>';
                    }
                } else {
