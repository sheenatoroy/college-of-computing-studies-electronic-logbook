...
<head>
    ...
    <title>CCS: E-LOG | Set Appointment</title>
<style>
body{
                ...
                <div class="row justify-content-center">
                    ...
                    <?php
                        $sql = "SELECT 
                                    ...
                                    m.end_time,
                                    m.day,
                                    GROUP_CONCAT(DISTINCT CONCAT(m.day, ' - ', m.subject_type) ORDER BY m.day, m.subject_type) AS schedule_types
                                FROM subj_management m
                                JOIN prof p ON p.username = m.status
                                WHERE m.status = '$username'
                                GROUP BY m.subject_code, m.year_section, m.status";
                                
                                $result = mysqli_query($conn, $sql);
                    ?>

                        <div class="alert alert-secondary text-dark shadow-sm rounded p-4 mt-3">
                            <h4 class="pt-2 mb-3"><i class='bx bx-card mx-2'></i>Student List</h4>
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    <div class="col">
                                        <div class="card text-dark bg-light mb-3 shadow-sm rounded" style="max-width: 25rem;">
                                            <div class="card-header"><span class="fw-bold fs-5"><?= $row['year_section'] ?></span></div>
                                            <div class="card-body">
                                                <h6 class="card-title mb-2">Course code: <br> <h2 class="fw-bold fs-4"><?= $row['subject_code'] ?></h2></h6>
                                                <p class="card-text">
                                                    Subject name: <h6><?php echo $row['subject_name'] ?></h6> <br>
                                                    Time: <h5><?php echo date('h:i A', strtotime($row['start_time'])) . ' - ' . date('h:i A', strtotime($row['end_time']));  ?></h5>
                                                    Schedule:
                                                    <?php
                                                    $scheduleTypes = explode(',', $row['schedule_types']);
                                                    foreach ($scheduleTypes as $scheduleType) {
                                                        echo "<h5>$scheduleType</h5>";
                                                    }
                                                    ?>
                                                </p>
                                                <a href="./functions/student-list.php?year_section=<?= $row['year_section'] ?>&subject_id=<?= $row['subject_id'] ?>" class="btn text-light mt-3" style="background-color: #F56904;">View List</a>
                                            </div>
                                        </div>
                                    </div>
                                    <?php
...
<!-- Modals -->
<?php include '../admin/functions/header-modals.php'; ?>
