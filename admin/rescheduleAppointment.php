...
// Gmail and PDF
require_once('../admin/functions/tcpdf/tcpdf.php');
require_once('../admin/functions/phpmailer/src/PHPMailer.php');
require_once('../admin/functions/phpmailer/src/SMTP.php');
require_once('../admin/functions/phpmailer/src/Exception.php');
...
if (isset($_POST['updateAppointment'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (mysqli_num_rows($pending_check_result) > 0) {
            echo "<script>alert('The selected day and time are not available!'); window.location.href = 'admin-pending-appointment.php'; </script>";
        } else if(mysqli_num_rows($same_day_result) > 0){
            echo "<script>alert('You are already schedule during this time and day!'); window.location.href = 'admin-pending-appointment.php'; </script>";
        }else{
                ...
                if ($student_details_result && mysqli_num_rows($student_details_result) > 0 && 
                    ...
                    if ($mail->send()) {
                        ...
                        if (mysqli_stmt_execute($update_status_stmt)) {
                            echo '<script>alert("Reschedule Appointment Successfully!"); window.location.href = "admin-pending-appointment.php";</script>';
                        } else {
